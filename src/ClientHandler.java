                ...
                switch (requestItems[0]) {
                    ...
                    case "GET":
                         ...
                         */
                            ...
                            file = new File(new String("index.html"));
                        } else {
                            fileName = requestItems[1].replaceFirst("/", "");
                            fileName = URLDecoder.decode(fileName, "UTF-8");

                            String[] url = fileName.split("/");
                            System.out.println("debug " + url[0]);
                            /*
                             * cgi-bin
                             */
                            //file = new File(fileName);
                            if (url[0].equals("cgi-bin")) {
                                System.out.println("Here");
                                String scriptName = null;
                                if (url.length != 1) {
                                   scriptName  = url[1];
                                } else {
                                    scriptName = "helloworld.pl";
                                }
                                File infile = new File(scriptName);
                                ...
                                HashMap<String, String> args = null;
                                 if (url.length != 1) {
                                     args = getQueryMap (url[1]);
                                 } else {
                                     args = new HashMap<>();
                                     args.put("name", "default");
                                 }
                                 ScriptExecute script = new ScriptExecute();

                                 file = script.execute(args, infile);
                                System.out.println(file.getName());
